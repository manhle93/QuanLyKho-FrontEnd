<template>
  <div class="app-container">
    <h4><i style="color: green"> MÓN ĂN BẾP NHÀ RUỘNG</i></h4>
    ...
    >
      ...
      <el-table-column
        ...
        label="Danh mục"
      ></el-table-column>
      <el-table-column prop="gia_ban" min-width="100" label="Giá bán">
        <template slot-scope="scope">{{
...
import { addMonBepNhaRuong, getMonBepNhaRuong } from "@/api/caidat";
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  methods: {
    ...
    async getMonBepNhaRuong() {
      this.listLoading = true;
      let data = await getMonBepNhaRuong();
      this.list = data;
    ...
    async addSanPham() {
      ...
      try {
        let data = await addMonBepNhaRuong({ data: this.monNgon });
        this.$message({
