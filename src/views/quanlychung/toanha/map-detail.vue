...
export default {
  ...
  methods: {
    ...
    setMarker(item) {
      document.getElementById("suggest").style.display = "none";
      this.deleteMarker();
      var lat = JSON.parse(item.coordinate).coordinates[1];
      var lng = JSON.parse(item.coordinate).coordinates[0];
    ...
    getLatLng(search) {
      axios
        .get(
          "https://auth.skysmartmap.com/api/auth/newgeocoding?q=" +
            search +
            "&api_key=FyMKEwsMQFBvVNOCVCxMpKACwkSyCl"
        )
        .then(res => {
          this.dataAddresses = res.data.result;
