<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="STT" width="95" align="center">
        <template slot-scope="scope">{{ scope.$index +1 }}</template>
      </el-table-column>
      <el-table-column sortable prop="code" label="Mã" width="150">
        <template slot-scope="scope">{{ scope.row.code }}</template>
      </el-table-column>
      <el-table-column sortable prop="name" label="Tên tỉnh, thành phố">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="110" fixed="right" label="Hoạt động">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    async fetchData(page = 1, per_page = 10) {
      ...
      getTinhThanh({ page: this.page, per_page: this.per_page }).then(response => {
   ...
   async searchData(page = 1, per_page = 10) {
    ...
    deleteAppTinhID(index, item) {
      this.$confirm("Bạn chắc chắn muốn xóa tỉnh thành này?", "Xóa tỉnh thành", {
