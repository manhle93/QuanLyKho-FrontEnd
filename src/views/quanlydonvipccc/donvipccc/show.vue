<template>
  <div class="app-container">
    <h4>Cập nhật thông tin đơn vị PCCC</h4>
            ...
            <el-col :md="{ span: 12 }">
              ...
              >
                <el-input
                  ...
                  placeholder="Nhập tổng số nhân sự"
          ...
          <el-row :gutter="20">
            <el-col :md="{ span: 12 }">
              <!-- <el-form-item label="Địa chỉ" prop="dia_chi">
                <el-input type="textarea" v-model="form.dia_chi" placeholder="Nhập vĩ độ"></el-input>
              </el-form-item>-->
              <el-form-item label="Loại hình đơn vị" prop="loai_hinh_don_vi">
                <br />
                <el-select v-model="form.loai_hinh_don_vi" style="width: 100%">
                  <el-option value="hanh_chinh" label="Hành chính"
                    >Đơn vị hành chính</el-option
                  >
                  <el-option value="nghiep_vu" label="Nghiệp vụ"
                    >Đơn vị nghiệp vụ</el-option
                  >
                </el-select>
              </el-form-item>
            </el-col>
...
import { editDonViPccc, getDonViDetail } from "@/api/donvipccc";
import { getTinhThanh } from "@/api/TinhThanh";
import { getQuanHuyen } from "@/api/QuanHuyen";
import { getInfor } from "@/api/taikhoan";
import BackButton from "@/layout/back";
import { userAvatar } from "@/api/donvipccc.js";
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        loai_hinh_don_vi: [
          {
            required: true,
            message: "Hãy chọn một loại hình đơn vị",
            trigger: "blur"
          }
        ],
        so_dien_thoai: [
  ...
  methods: {
    handleChange(e) {
      ...
      if (!dinhDangChoPhep.find(el => el == filePath)) {
        ...
        userAvatar(this.$route.params.id, data)
          .then(res => {
    ...
    submit(createInfo) {
      this.$refs[createInfo].validate(valid => {
        if (valid) {
          this.loading = true;
          if (this.form.avatar_url === null || this.form.avatar_url === "") {
            this.src =
              process.env.VUE_APP_BASE + "/storage/images/avatar/firehat.png";
          } else this.src = process.env.VUE_APP_BASE + this.form.avatar_url;
          editDonViPccc(this.$route.params.id, this.form)
            .then(res => {
              this.loading = false;
              this.$message({
    ...
    async getDonViPcccDetail(id) {
      let donvi = await getDonViDetail(id);
      this.donvi = donvi.data;
      if (this.donvi.avatar_url) {
        this.src = process.env.VUE_APP_BASE + "/" + this.donvi.avatar_url;
      } else {
        this.src =
          process.env.VUE_APP_BASE + "/storage/images/avatar/firehat.png";
      }
      for (let field in this.form) {
        this.form[field] = this.donvi[field];
      }
  ...
  created() {
    this.getDonViPcccDetail(this.$route.params.id);
