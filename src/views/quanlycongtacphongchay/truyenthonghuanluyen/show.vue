    ...
    <el-form ref="form" :model="form" :rules="rules">
      <el-row :gutter="20">
        <el-col :span="22" :offset="1">
          <h4>Cập nhật công tác tuyên truyền, huấn luyện, bồi dưỡng nghiệp vụ PCCC</h4>
          <el-row :gutter="20">
            <el-col :sm="{span:6}">
            ...
            <el-col :sm="{span:6}">
              <el-form-item label="Thời lượng (giờ)" prop="thoi_luong">
                <el-input
                  v-model="form.thoi_luong"
                  placeholder="Thời lượng huấn luyện, bồi dưỡng nghiệp vụ"
                ></el-input>
              </el-form-item>
            </el-col>
            <el-col :sm="{span:18}">
...
import {
  updateHuanLuyenBoiDuong,
  showHuanLuyenBoiDuong
} from "@/api/huanluyenboiduong";
...
export default {
  ...
  methods: {
    async getData(id) {
      let data = await showHuanLuyenBoiDuong(id);
      this.form = data;
      if (data.tinh_thanh_id) {
    ...
    submit(createInfo) {
      this.$refs[createInfo].validate(valid => {
        if (valid) {
          this.loading = true;
          updateHuanLuyenBoiDuong(this.$route.params.id, this.form)
            .then(res => {
              this.loading = false;
              this.$message({
