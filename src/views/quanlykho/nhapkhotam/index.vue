<template>
  <div class="app-container">
    <h4><i style="color: green">DANH SÁCH NHẬP KHO TẠM</i></h4>
    <el-row :gutter="20" justify="space-around">
      ...
      <el-col :span="6">
        <el-button
          ...
          icon="el-icon-plus"
          @click="showNhapKho()"
          >THÊM PHIẾU NHẬP KHO TẠM</el-button
        >
      </el-col>
    </el-row>
    <br />
    ...
    >
      ...
      <el-table-column label="Ghi chú">
        <template slot-scope="scope">
          ...
          <div
            v-if="
              scope.row.don_hang &&
                scope.row.don_hang.trang_thai == 'nhap_kho_tam' && scope.row.trang_thai == 1
            "
          >
            <div style="color: green; font-weight: bold">
              Nhập kho tạm chờ hàng về
            </div>
            <span>{{ scope.row.ghi_chu }}</span>
          </div>

          <div
            v-if="scope.row.trang_thai == 2"
          >
            <div style="color: #409eff; font-weight: bold">
              Hàng đã được đưa vào kho thành công
            </div>
            <span>{{ scope.row.ghi_chu }}</span>
          </div>

          <div
            v-if="scope.row.trang_thai == 3"
          >
            <div style="color: red; font-weight: bold">
              Hàng đã được xóa khỏi kho tạm
            </div>
            <span>{{ scope.row.ghi_chu }}</span>
          </div>
      ...
      <el-table-column align="center" fixed="right" label="Chốt kho hàng">
        <template slot-scope="scope">
          <!-- <el-tooltip
            class="item"
            effect="dark"
          ...
          >
            <el-button
              v-if="scope.row.trang_thai == 1"
              size="small"
              icon="el-icon-check"
              type="success"
              circle
              @click="chotKhoTam(scope.row.id, scope.row.don_hang)"
            ></el-button>
          </el-tooltip>
          <el-tooltip
            class="item"
          ...
          >
            <el-button
              ...
              circle
              type="danger"
              @click="huyKhoTam(scope.row.id, scope.row.don_hang)"
            ></el-button>
          </el-tooltip>
    ...
    <el-dialog
      ...
      title="NHẬP HÀNG HÓA BỔ SUNG VÀO KHO TẠM"
      width="800px"
      center
...
<script>
import { getNhapKhoTam, nhapKhoTam, chotKhoTam, huyKhoTam} from "@/api/quanlykho";
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        let data = await nhapKhoTam(this.formAdd);
        this.getData();
